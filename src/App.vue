...
const copy = computed(() => language.value === 'en' ? {
  docs: 'Docs', technology: 'Capabilities', product: 'Screens', architecture: 'Architecture',
  experience: 'Open demo', heroEyebrow: 'J2AGENT AI · INTELLIGENT WORK PLATFORM',
  heroTitle: ['Turn foundation models into', 'real work.'],
  heroBody: 'J2Agent connects agents, tools, enterprise knowledge, and an observable runtime, taking AI from “can answer” to “can complete”.',
  viewScreens: 'View product screens', technologyEyebrow: 'ENGINEERED FOR REAL WORK',
  technologyTitle: ['Not just model access,', 'a complete system.'],
  technologyBody: 'Every layer is designed for controllability, traceability, and extensibility in real business workflows.',
  productEyebrow: 'THE PLATFORM IN ACTION', productTitle: ['See every step,', 'as it happens.'],
  productBody: 'One glass workspace for conversations, knowledge bases, retrieval, agent permissions, and audit.',
  architectureEyebrow: 'ONE TRACE, FULL CONTROL', architectureTitle: ['Every answer', 'has a trace.'],
  architectureBody: 'A unified event envelope connects thinking, tool calls, streaming text, and completion. Sources, tool calls, token usage, and sessions stay auditable.',
  readArchitecture: 'Read architecture docs', runtime: 'Running task', intent: 'Intent recall', knowledge: 'Knowledge base call', answer: 'Streaming answer', done: 'Done', active: 'In progress', input: 'Find the best route from Wuhan to Shenzhen',
  footer: 'Agent platform · Turn knowledge into executable power', noMatch: 'No matching documents', loading: 'Loading document…', searchDocs: 'Search indexed docs', path: 'Enter any .md path', open: 'Open', close: 'Close docs', github: 'View on GitHub', docsTitle: 'Documentation'
} : {
  docs: '文档中心', technology: '技术能力', product: '产品界面', architecture: '平台架构',
  experience: '进入体验', heroEyebrow: 'J2AGENT AI · INTELLIGENT WORK PLATFORM',
  heroTitle: ['把大模型的能力，', '接入真实工作。'],
  heroBody: 'J2Agent 将 Agent、工具、企业知识与可观测运行时连接起来，让 AI 从“会回答”走向“能完成”。',
  viewScreens: '查看产品界面', technologyEyebrow: 'ENGINEERED FOR REAL WORK',
  technologyTitle: ['不是简单接入模型，', '而是一套完整系统。'],
  technologyBody: '从检索质量到任务可靠性，每一层都为真实业务中的可控、可追溯与可扩展而设计。',
  productEyebrow: 'THE PLATFORM IN ACTION', productTitle: ['一眼看见，', '每一步都在发生。'],
  productBody: '统一的玻璃化工作台，承载对话、知识库、检索配置、智能体权限与审计能力。',
  architectureEyebrow: 'ONE TRACE, FULL CONTROL', architectureTitle: ['每一个回答，', '都有迹可循。'],
  architectureBody: '统一事件信封串起 THINKING、CALLING_TOOL、STREAMING_TEXT 到 COMPLETED 的完整状态迁移；来源、工具调用、Token 用量与会话记录，都可以被审计和复盘。',
  readArchitecture: '阅读架构文档', runtime: '正在执行任务', intent: '意图召回', knowledge: '调用知识库', answer: '流式生成答案', done: '完成', active: '进行中', input: '查找武汉到深圳的最优方案',
  footer: '智能体平台 · 让知识成为可执行的力量', noMatch: '没有匹配的文档', loading: '正在加载正文…', searchDocs: '搜索已收录文档', path: '输入任意 .md 路径', open: '打开', close: '关闭文档', github: '在 GitHub 查看', docsTitle: '文档中心'
})
const toggleLanguage = () => { language.value = language.value === 'zh' ? 'en' : 'zh'; document.documentElement.lang = language.value === 'en' ? 'en' : 'zh-CN' }
...
const features = computed(() => language.value === 'en' ? {
  rag: { label: 'RAG retrieval', title: 'Ground every answer', body: 'From document ingestion and chunking to vectorization, recall, and source display, J2Agent forms a complete knowledge-enhancement chain.', chips: ['Dense vectors', 'BM25 sparse retrieval', 'Milvus hybrid ranking'] },
  agent: { label: 'Agent orchestration', title: 'Let agents collaborate', body: 'The general assistant recalls intent and makes decisions, then delegates to specialist agents when needed through one streaming path.', chips: ['Multi-agent orchestration', 'ReAct tool loop', 'Skill / MCP extensions'] },
  runtime: { label: 'Reliable runtime', title: 'Keep tasks traceable', body: 'The UI is only an observer. Background tasks keep running through refreshes and disconnects, with snapshot recovery available.', chips: ['WebSocket state machine', 'Redis queue & Pub/Sub', 'Snapshot resume'] }
} : {
...
<template>
  <div class="site-shell" :class="{ 'docs-mode': readerOpen }">
    <header class="topbar glass-panel">
      ...
      <nav><a href="#docs" class="docs-link" @click.prevent="openReader">{{ copy.docs }}</a><a href="#technology">{{ copy.technology }}</a><a href="#product">{{ copy.product }}</a><a href="#architecture">{{ copy.architecture }}</a></nav>
      <div class="topbar-actions"><button class="language-toggle" type="button" @click="toggleLanguage">{{ language === 'zh' ? 'EN' : '中文' }}</button><button class="glass-button blue" type="button" @click="openReader">{{ copy.docs }} <span>↗</span></button></div>
    </header>

    <main id="top">
      <section class="hero section-wrap">
        <div class="hero-copy"><div class="eyebrow"><i></i> {{ copy.heroEyebrow }}</div><h1>{{ copy.heroTitle[0] }}<br /><span>{{ copy.heroTitle[1] }}</span></h1><p>{{ copy.heroBody }}</p><div class="hero-actions"><a class="glass-button blue large" href="https://j2agent.jerryt92.top/" target="_blank" rel="noreferrer">{{ copy.experience }} <b>↗</b></a><button class="link-button" @click="jump('product')">{{ copy.viewScreens }} <b>↓</b></button></div><div class="proof-row"><span>SPRING AI</span><span>WEBSOCKET</span><span>MILVUS</span><span>REDIS</span></div></div>
        <div class="hero-art"><div class="orb orb-blue"></div><div class="orb orb-orange"></div><div class="hero-glass glass-panel"><div class="hero-glass-head"><span class="live-dot"></span> AGENT RUNTIME <em>LIVE</em></div><div class="runtime-state"><div class="state-ring">✦</div><div><strong>{{ copy.runtime }}</strong><small>universal_assistant · turn_2048</small></div><span class="state-time">00:12</span></div><div class="trace"><div class="trace-item done"><i>✓</i><span>{{ copy.intent }}</span><b>{{ copy.done }}</b></div><div class="trace-item done"><i>✓</i><span>{{ copy.knowledge }}</span><b>48 docs</b></div><div class="trace-item active"><i>↗</i><span>{{ copy.answer }}</span><b>{{ copy.active }}</b></div></div><div class="runtime-input">{{ copy.input }} <button>↑</button></div></div></div>
      </section>

      <section id="technology" class="technology section-wrap"><div class="section-intro"><div><div class="eyebrow">{{ copy.technologyEyebrow }}</div><h2>{{ copy.technologyTitle[0] }}<br /><span>{{ copy.technologyTitle[1] }}</span></h2></div><p>{{ copy.technologyBody }}</p></div><div class="tech-layout"><div class="tech-tabs"><button v-for="(feature, key) in features" :key="key" :class="{ selected: activeFeature === key }" @click="activeFeature = key"><span>0{{ Object.keys(features).indexOf(key) + 1 }}</span><strong>{{ feature.label }}</strong><b>↗</b></button></div><div class="tech-detail glass-panel"><div class="detail-kicker">{{ features[activeFeature].label }}</div><h3>{{ features[activeFeature].title }}</h3><p>{{ features[activeFeature].body }}</p><div class="chip-row"><span v-for="chip in features[activeFeature].chips" :key="chip">{{ chip }}</span></div><div class="detail-diagram"><div class="diagram-node">输入</div><i>→</i><div class="diagram-node blue-node">{{ activeFeature === 'rag' ? 'Hybrid Retrieval' : activeFeature === 'agent' ? 'Orchestrator' : 'Background Turn' }}</div><i>→</i><div class="diagram-node">结果</div></div></div></div></section>

      <section id="product" class="product section-wrap"><div class="section-intro"><div><div class="eyebrow">{{ copy.productEyebrow }}</div><h2>{{ copy.productTitle[0] }}<br /><span>{{ copy.productTitle[1] }}</span></h2></div><p>{{ copy.productBody }}</p></div><div class="screen-grid"><figure class="screen-card wide"><img src="/screens/02-chat-analysis.png" alt="J2Agent 通用 AI 助手与分析结果界面" /><figcaption><strong>通用 AI 助手</strong><span>调用工具，流式输出分析结果</span></figcaption></figure><figure class="screen-card"><img src="/screens/01-task-list.png" alt="J2Agent 智能体任务列表界面" /><figcaption><strong>任务与执行轨迹</strong><span>多任务状态实时可见</span></figcaption></figure><figure class="screen-card"><img src="/screens/04-kb-sync.png" alt="J2Agent 知识库同步状态界面" /><figcaption><strong>知识库同步</strong><span>增量维护，状态清晰可追溯</span></figcaption></figure><figure class="screen-card"><img src="/screens/06-rag-settings.png" alt="J2Agent RAG 检索设置界面" /><figcaption><strong>RAG 参数配置</strong><span>稠密与稀疏权重可调</span></figcaption></figure><figure class="screen-card"><img src="/screens/03-kb-list.png" alt="J2Agent 知识库列表界面" /><figcaption><strong>知识库管理</strong><span>远程仓库与访问权限统一管理</span></figcaption></figure><figure class="screen-card"><img src="/screens/05-chat-audit.png" alt="J2Agent 聊天审计界面" /><figcaption><strong>Token 与会话审计</strong><span>跨用户记录可查询、可复盘</span></figcaption></figure><figure class="screen-card"><img src="/screens/07-agent-management.png" alt="J2Agent 智能体管理界面" /><figcaption><strong>智能体管理</strong><span>权限、插件与全局配置集中维护</span></figcaption></figure></div></section>

      <section id="architecture" class="architecture section-wrap"><div class="arch-copy"><div class="eyebrow">{{ copy.architectureEyebrow }}</div><h2>{{ copy.architectureTitle[0] }}<br /><span>{{ copy.architectureTitle[1] }}</span></h2><p>{{ copy.architectureBody }}</p><button class="glass-button" @click="openReader">{{ copy.readArchitecture }} <span>↗</span></button></div><div class="arch-stack"><div class="stack-card"><span class="stack-num">01</span><strong>Agent Layer</strong><small>AiAgent · ReAct · 子智能体编排</small></div><div class="stack-card offset"><span class="stack-num">02</span><strong>Knowledge Layer</strong><small>Query Transformer · Hybrid Retrieval · RAG</small></div><div class="stack-card"><span class="stack-num">03</span><strong>Runtime Layer</strong><small>WebSocket · Redis Queue · Snapshot Resume</small></div></div></section>
    </main>
    <section v-if="readerOpen" id="docs" class="reader-overlay" role="dialog" aria-modal="true" aria-label="J2Agent 文档中心">
      <div class="reader-shell glass-panel">
        <header class="reader-head"><div><div class="eyebrow">J2AGENT DOCS · GITHUB</div><h2>{{ copy.docsTitle }}</h2></div><div class="reader-actions"><a href="https://github.com/j2agent-ai/j2agent-docs" target="_blank" rel="noreferrer" class="repo-link">{{ copy.github }} ↗</a><button class="close-reader" :aria-label="copy.close" @click="closeReader">×</button></div></header>
        <div class="reader-body"><aside class="doc-nav"><input v-model="docSearch" type="search" :placeholder="copy.searchDocs" :aria-label="copy.searchDocs" /><div class="path-loader"><input v-model="docPath" type="text" :placeholder="copy.path" aria-label="输入任意 Markdown 路径" @keyup.enter="loadDocByPath" /><button @click="loadDocByPath">{{ copy.open }}</button></div><button v-for="doc in filteredDocs" :key="doc.path" :class="{ active: selectedDoc === doc.path }" @click="loadDoc(doc.path)"><span>{{ doc.path.includes('/') ? '·' : '⌂' }}</span>{{ doc.title }}</button><div v-if="!filteredDocs.length" class="doc-state">{{ copy.noMatch }}</div></aside><article class="markdown-view" @click="handleMarkdownClick"><div v-if="docError" class="doc-notice">{{ docError }}</div><div v-if="docLoading" class="doc-state">{{ copy.loading }}</div><div v-else v-html="docHtml"></div></article></div>
      </div>
    </section>
    <footer><img src="/logo-b.svg" alt="J2Agent AI" /><span>{{ copy.footer }}</span><span class="footer-right">© 2026 J2Agent</span></footer>
    <KbQaWidget :language="language" @open-doc="openDocFromAi" />
